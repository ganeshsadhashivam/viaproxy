...
const initialValues: SubmitExchangeDetails = {
  submitExchangeDetails: {
    ...
    materialConditions: {
      ...
      depositPayment: { decision: "", depositPayment: {} },
...
const SubmitExchangeValidationSchema = Yup.object({
  submitExchangeDetails: Yup.object({
    ...
    materialConditions: Yup.object({
      ...
      depositPayment: Yup.object().shape({
        decision: Yup.string()
          .oneOf(["yes", "no", ""], "Invalid decision")
          .required("Decision is required"),
        depositPayment: Yup.object().when(
          "decision",
          (decision: unknown, schema) => {
            if (decision === "yes") {
              return schema.shape({
                percentage: Yup.string()
                  .required("Percentage is required when decision is 'yes'")
                  .matches(
                    /^\d+(\.\d{1,2})?$/,
    ...
    deliveryConditions: Yup.object({
      pickup: Yup.object({
        allowed: Yup.string()
          .required("Please specify if pickup is allowed")
      ...
      delivery: Yup.object({
        allowed: Yup.string()
          .required("Please specify if delivery is allowed")
          .oneOf(["yes", "no"], "Please select Yes or No"),
        costOption: Yup.string().when("allowed", (allowed: unknown) => {
          if (allowed === "yes") {
            return Yup.string()
              .required("Please specify if there is a delivery cost")
              .oneOf(["yes", "no"], "Please select Yes or No");
          }
        ...
        details: Yup.object().when("allowed", (allowed: unknown) => {
          if (allowed === "yes") {
            return Yup.object({
              amount: Yup.string().when("costOption", (costOption: unknown) => {
                if (costOption === "yes") {
                  return Yup.string().required(
                    "Delivery cost amount is required"
                  );
                }
                return Yup.string().nullable();
              }),
...
//     console.log(
//       "Form Values Collected in Parent:",
//       expectedRequirementFormValue
//     );
